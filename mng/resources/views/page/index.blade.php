...
<head>
	...
	<script src="{{ asset('/js/Page/index.js')  . $ver_str }}" defer></script>
...
<div class="cb_func_line">
	...
	<!-- 検索条件入力フォーム -->
	<form action="" class="form_kjs" id="pageIndexForm" method="post" accept-charset="utf-8">
		
		<?php $this->CrudBase->inputKjMain('kj_main','',null,'ページ名、備考を検索する');?>
		...
		$this->CrudBase->inputKjText('kj_page_name','ページ名');
...
<!-- 一覧テーブル -->
<table id="page_tbl" class="table table-striped table-bordered table-condensed" style="display:none;margin-bottom:0px">
...
foreach($data as $i=>&$ent){
	...
	$this->CrudBase->tdStr($ent, 'page_name');
...
<table id="crud_base_forms">
	...
	<tr id="ajax_crud_new_inp_form" class="crud_base_form" style="display:none;padding-bottom:60px"><td colspan='5'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp' >ページ名: </div>
			<div class='cbf_input'>
				<input type="text" name="page_name" class="valid " value=""  maxlength="100" title="100文字以内で入力してください" />
				<label class="text-danger" for="page_name"></label>
			</div>
		</div>
