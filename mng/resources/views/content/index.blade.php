...
<head>
	...
	<title>コンテンツ管理画面</title>
	
	<script src="{{ asset('/js/app.js') }}" defer></script>
	<script src="{{ $crud_base_js . $ver_str }}" defer></script>
	<script src="{{ asset('/js/Content/index.js')  . $ver_str }}" defer></script>
...
<div class="cb_func_line">
	...
	<!-- 検索条件入力フォーム -->
	<form action="" class="form_kjs" id="contentIndexForm" method="post" accept-charset="utf-8">
		
		<?php $this->CrudBase->inputKjMain('kj_main','',null,'コンテンツ名、備考を検索する');?>
		...
		$this->CrudBase->inputKjSelect('kj_page_id','ページID', $masters['pageIdList']); 
		$this->CrudBase->inputKjText('kj_code','コード');
		$this->CrudBase->inputKjText('kj_content_name','コンテンツ名');
		$this->CrudBase->inputKjText('kj_text1','テキスト1');
		$this->CrudBase->inputKjText('kj_midasi1','見出し1');
		$this->CrudBase->inputKjSelect('kj_img_flg','画像タイプフラグ', $masters['imgFlgList']); 
		$this->CrudBase->inputKjText('kj_img_fp','画像ファイルパス');
		$this->CrudBase->inputKjOuterId('kj_img_id','画像ファイルID'); 
	...
	<div id="main_tools" style="margin-bottom:10px;margin-top:4px">
		<div style="display:inline-block;width:75%; ">
			<?php 
				...
				// CSVエクスポート機能
	 			$csv_dl_url =  'content/csv_download';
...
<!-- 一覧テーブル -->
<table id="content_tbl" class="table table-striped table-bordered table-condensed" style="display:none;margin-bottom:0px">
...
foreach($data as $i=>&$ent){
	...
	$this->CrudBase->tdList($ent, 'page_id', $pageIdList);
	$this->CrudBase->tdStr($ent, 'code');
	$this->CrudBase->tdStr($ent, 'content_name');
	$this->CrudBase->tdNote($ent, 'text1');
	$this->CrudBase->tdStr($ent, 'midasi1');
	$this->CrudBase->tdList($ent, 'img_flg', $imgFlgList);
	$this->CrudBase->tdImage($ent, 'img_fp');
	$this->CrudBase->tdOuterName($ent, 'img_id', 'img_name');
...
<table id="crud_base_forms">
	...
	<tr id="ajax_crud_new_inp_form" class="crud_base_form" style="display:none;padding-bottom:60px"><td colspan='5'>
		...
		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >ページID: </div>
			<div class='cbf_input'>
				<?php $this->CrudBase->selectX('page_id',null,$pageIdList,null);?>
				<label class="text-danger" for="page_id"></label>
			</div>
		</div>

		<div class="cbf_inp_wrap">
			<div class='cbf_inp' >コード: </div>
			<div class='cbf_input'>
				<input type="text" name="code" class="valid " value=""  maxlength="16" title="16文字以内で入力してください" />
				<label class="text-danger" for="code"></label>
			</div>
		</div>

		<div class="cbf_inp_wrap">
			<div class='cbf_inp' >コンテンツ名: </div>
			<div class='cbf_input'>
				<input type="text" name="content_name" class="valid " value=""  maxlength="100" title="100文字以内で入力してください" />
				<label class="text-danger" for="content_name"></label>
			</div>
		</div>

		<div class="cbf_inp_wrap_long">
			<div class='cbf_inp_label' >テキスト1： </div>
			<div class='cbf_input'>
				<textarea name="text1" maxlength="1000" title="1000文字以内で入力してください" style="height:100px;width:100%"></textarea>
				<label class="text-danger" for="text1"></label>
			</div>
		</div>
		<div class="cbf_inp_wrap">
			<div class='cbf_inp' >見出し1: </div>
			<div class='cbf_input'>
				<input type="text" name="midasi1" class="valid " value=""  maxlength="200" title="200文字以内で入力してください" />
				<label class="text-danger" for="midasi1"></label>
			</div>
		</div>

		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >画像タイプフラグ: </div>
			<div class='cbf_input'>
				<?php $this->CrudBase->selectX('img_flg',null,$imgFlgList,null);?>
				<label class="text-danger" for="img_flg"></label>
			</div>
		</div>

		<div class="cbf_inp_wrap" style="float:left">
			<div class='cbf_inp_label_long' >画像ファイルパス: </div>
			<div class='cbf_input' style="width:180px;height:auto;">
				<label for="img_fp_n" class="fuk_label" >
					<input type="file" id="img_fp_n" class="img_fp" style="display:none" accept="image/*" title="画像ファイルをドラッグ＆ドロップ(複数可)"  data-inp-ex='image_fuk' data-fp='' />
					<span class='fuk_msg' style="padding:20%">画像ファイルをドラッグ＆ドロップ(複数可)</span>
				</label>
			</div>
		</div>
		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >画像ファイルID: </div>
			<?php $this->CrudBase->formOuterName('img_id', '画像ファイルID', 'ni'); ?>
		</div>
	...
	<tr id="ajax_crud_edit_form" class="crud_base_form" style="display:none"><td colspan='5'>
		...
		<div class="cbf_inp_wrap_long">
			<div class='cbf_inp_label' >テキスト1： </div>
			<div class='cbf_input'>
				<textarea name="text1" maxlength="1000" title="1000文字以内で入力してください" data-folding-ta="40" style="height:100px;width:100%"></textarea>
				<label class="text-danger" for="text1"></label>
			</div>
		</div>
		...
		<div class="cbf_inp_wrap" style="float:left">
			<div class='cbf_inp_label_long' >画像ファイルパス: </div>
			<div class='cbf_input' style="width:180px;height:auto;">
				<label for="img_fp_e" class="fuk_label" >
					<input type="file" id="img_fp_e" class="img_fp" style="display:none" accept="image/*" title="画像ファイルをドラッグ＆ドロップ(複数可)"  data-inp-ex='image_fuk' data-fp='' />
					<span class='fuk_msg' style="padding:20%">画像ファイルをドラッグ＆ドロップ(複数可)</span>
				</label>
			</div>
		</div>
		<div class="cbf_inp_wrap">
			<div class='cbf_inp_label' >画像ファイルID: </div>
			<?php $this->CrudBase->formOuterName('img_id', '画像ファイルID', 'edit'); ?>
		</div>
			<div class="cbf_inp_wrap">
