...
jQuery(()=> {
	...
	$('#content_tbl').show();// 高速表示のためテーブルは最後に表示する
...
function init(){
	...
	// CBBXS-2023
	// ページIDリストJSON
	let pageIdList = crudBaseData.masters.pageIdList;
	disFilData['page_id'] ={'fil_type':'select','option':{'list':pageIdList}};
	// 画像タイプフラグリストJSON
	let imgFlgList = crudBaseData.masters.imgFlgList;
	disFilData['img_flg'] ={'fil_type':'select','option':{'list':imgFlgList}};
	...
	crudBase.crudBaseBulkAdd.init(
		[
			...
			{'field':'page_id', 'inp_type':'select', 'list':pageIdList, 'def':0}, 
			{'field':'img_flg', 'inp_type':'select', 'list':imgFlgList, 'def':0}, 
			{'field':'img_id', 'inp_type':'textarea'}, 
			{'field':'sort_no', 'inp_type':'textarea'}, 
			{'field':'delete_flg', 'inp_type':'textarea'}, 

			// CBBXE
			
//			{'field':'content_group', 'inp_type':'select', 'list':contentGroupList, 'def':2}, 
//			{'field':'content_date', 'inp_type':'date', 'def':today}, 
//			{'field':'note', 'inp_type':'text', 'def':'TEST'}, 
//			{'field':'sort_no', 'inp_type':'sort_no', 'def':1}, 
		],
		{
			ajax_url:'content/bulk_reg',
			csrf_token:csrf_token,
			ta_placeholder:"Excelからコピーしたコンテンツ名、コンテンツ数値を貼り付けてください。（タブ区切りテキスト）\n(例)\nコンテンツ名A\t100\nコンテンツ名B\t101\n",
		}
	);
...
function _getValidMethods(){
	let methods = {
			...
			kj_code:(cbv, value)=>{
				let err = '';
				// 文字数バリデーション
				if(!cbv.isMaxLength(value, 16)){
					err = '16文字以内で入力してくだい。';
				}
				return err;
			},
			kj_content_name:(cbv, value)=>{
				let err = '';
				// 文字数バリデーション
				if(!cbv.isMaxLength(value, 100)){
					err = '100文字以内で入力してくだい。';
				}
				return err;
			},
			kj_text1:(cbv, value)=>{
				let err = '';
				// 文字数バリデーション
				if(!cbv.isMaxLength(value, 3000)){
					err = '3000文字以内で入力してくだい。';
				}
				return err;
			},
			kj_midasi1:(cbv, value)=>{
				let err = '';
				// 文字数バリデーション
				if(!cbv.isMaxLength(value, 200)){
					err = '200文字以内で入力してくだい。';
				}
				return err;
			},
...
function calendarViewKShow(){
	// カレンダービューを生成 
	crudBase.calendarViewCreate('content_date');
}
