...
jQuery(()=> {
	...
	$('#img_tbl').show();// 高速表示のためテーブルは最後に表示する
...
//			{'field':'img_group', 'inp_type':'select', 'list':imgGroupList, 'def':2}, 
//			{'field':'img_date', 'inp_type':'date', 'def':today}, 
//			{'field':'note', 'inp_type':'text', 'def':'TEST'}, 
//			{'field':'sort_no', 'inp_type':'sort_no', 'def':1}, 
		],
		{
			ajax_url:'img/bulk_reg',
			csrf_token:csrf_token,
			ta_placeholder:"Excelからコピーした画像名、画像数値を貼り付けてください。（タブ区切りテキスト）\n(例)\n画像名A\t100\n画像名B\t101\n",
		}
	);
...
function calendarViewKShow(){
	// カレンダービューを生成 
	crudBase.calendarViewCreate('img_date');
}
