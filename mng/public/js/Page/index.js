...
jQuery(()=> {
	...
	$('#page_tbl').show();// 高速表示のためテーブルは最後に表示する
...
//			{'field':'page_group', 'inp_type':'select', 'list':pageGroupList, 'def':2}, 
//			{'field':'page_date', 'inp_type':'date', 'def':today}, 
//			{'field':'note', 'inp_type':'text', 'def':'TEST'}, 
//			{'field':'sort_no', 'inp_type':'sort_no', 'def':1}, 
		],
		{
			ajax_url:'page/bulk_reg',
			csrf_token:csrf_token,
			ta_placeholder:"Excelからコピーしたページ名、ページ数値を貼り付けてください。（タブ区切りテキスト）\n(例)\nページ名A\t100\nページ名B\t101\n",
		}
	);
...
function calendarViewKShow(){
	// カレンダービューを生成 
	crudBase.calendarViewCreate('page_date');
}
