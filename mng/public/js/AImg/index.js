...
jQuery(()=> {
	...
	$('#a_img_tbl').show();// 高速表示のためテーブルは最後に表示する
...
//			{'field':'a_img_group', 'inp_type':'select', 'list':a_imgGroupList, 'def':2}, 
//			{'field':'a_img_date', 'inp_type':'date', 'def':today}, 
//			{'field':'note', 'inp_type':'text', 'def':'TEST'}, 
//			{'field':'sort_no', 'inp_type':'sort_no', 'def':1}, 
		],
		{
			ajax_url:'a_img/bulk_reg',
			csrf_token:csrf_token,
			ta_placeholder:"ExcelからコピーしたA画像名、A画像数値を貼り付けてください。（タブ区切りテキスト）\n(例)\nA画像名A\t100\nA画像名B\t101\n",
		}
	);
...
function calendarViewKShow(){
	// カレンダービューを生成 
	crudBase.calendarViewCreate('a_img_date');
}
