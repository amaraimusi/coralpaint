...
class AImgController extends AppController
{
	...
	public function index(){
		...
		return view('a_img.index', compact('data', 'crudBaseData', 'crud_base_json'));
	...
	public function ajax_reg(){
		...
		if(!empty($_FILES)){
			// CBBXS-2027
			$img_fp = $this->cb->makeFilePath($_FILES, "storage/AImg/y%Y/{$ent['id']}/%unique/orig/%fn", $ent, 'img_fp');
	...
	private function init(){
		...
		$fieldData = ['def'=>[
			...
			'id'=>[
					'name'=>'ID',//HTMLテーブルの列名
					'row_order'=>'AImg.id',//SQLでの並び替えコード
					'clm_show'=>1,//デフォルト列表示 0:非表示 1:表示
			],
			'img_fp'=>[
					'name'=>'画像ファイルパス',
					'row_order'=>'AImg.img_fp',
					'clm_show'=>1,
			],
			'sort_no'=>[
					'name'=>'順番',
					'row_order'=>'AImg.sort_no',
					'clm_show'=>0,
			],
			'delete_flg'=>[
					'name'=>'無効フラグ',
					'row_order'=>'AImg.delete_flg',
					'clm_show'=>0,
			],
			'update_user'=>[
					'name'=>'更新者',
					'row_order'=>'AImg.update_user',
					'clm_show'=>0,
			],
			'ip_addr'=>[
					'name'=>'IPアドレス',
					'row_order'=>'AImg.ip_addr',
					'clm_show'=>0,
			],
			'created'=>[
					'name'=>'生成日時',
					'row_order'=>'AImg.created',
					'clm_show'=>0,
			],
			'modified'=>[
					'name'=>'更新日',
					'row_order'=>'AImg.modified',
					'clm_show'=>0,
			],
		...
		$model = new AImg(); // モデルクラス
		
		$crudBaseData = [
				'fw_type' => 'laravel7',
				'model_name_c' => 'AImg',
				'tbl_name' => 'a_imgs', // テーブル名をセット
	...
	private function getDataForDownload(){
		...
		//セッションから検索条件情報を取得
		$kjs=session('a_img_kjs');

		// セッションからページネーション情報を取得
		$pages = session('a_img_pages');
