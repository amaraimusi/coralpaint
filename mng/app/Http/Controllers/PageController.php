...
class PageController extends AppController
{
	...
	public function index(){
		...
		return view('page.index', compact('data', 'crudBaseData', 'crud_base_json'));
	...
	private function init(){
		...
		$fieldData = ['def'=>[
			...
			'id'=>[
					'name'=>'ID',//HTMLテーブルの列名
					'row_order'=>'Page.id',//SQLでの並び替えコード
					'clm_show'=>1,//デフォルト列表示 0:非表示 1:表示
			],
			'page_name'=>[
					'name'=>'ページ名',
					'row_order'=>'Page.page_name',
					'clm_show'=>1,
			],
			'sort_no'=>[
					'name'=>'順番',
					'row_order'=>'Page.sort_no',
					'clm_show'=>0,
			],
			'delete_flg'=>[
					'name'=>'無効フラグ',
					'row_order'=>'Page.delete_flg',
					'clm_show'=>0,
			],
			'update_user'=>[
					'name'=>'更新者',
					'row_order'=>'Page.update_user',
					'clm_show'=>0,
			],
			'ip_addr'=>[
					'name'=>'IPアドレス',
					'row_order'=>'Page.ip_addr',
					'clm_show'=>0,
			],
			'created'=>[
					'name'=>'生成日時',
					'row_order'=>'Page.created',
					'clm_show'=>0,
			],
			'modified'=>[
					'name'=>'更新日',
					'row_order'=>'Page.modified',
					'clm_show'=>0,
			],
		...
		$model = new Page(); // モデルクラス
		
		$crudBaseData = [
				'fw_type' => 'laravel7',
				'model_name_c' => 'Page',
				'tbl_name' => 'pages', // テーブル名をセット
	...
	private function getDataForDownload(){
		...
		//セッションから検索条件情報を取得
		$kjs=session('page_kjs');

		// セッションからページネーション情報を取得
		$pages = session('page_pages');
