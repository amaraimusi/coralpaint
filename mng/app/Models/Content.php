...
class Content extends Model
{
	...
	private function createKjConditions($kjs){
		...
		if(!empty($kjs['kj_img_id']) || $kjs['kj_img_id'] ==='0' || $kjs['kj_img_id'] ===0){
			$cnds[]="Content.img_id = {$kjs['kj_img_id']}";
		}
	...
	/**
	 * 画像タイプフラグリストをDBから取得する
	 */
	public function getImgFlgList(){

		// DBからデータを取得
		$query = \DB::table('img_flgs')->
		whereRaw("delete_flg = 0")->
		orderBy('sort_no', 'ASC');
		$data = $query->get();
